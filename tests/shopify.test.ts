...
beforeAll(async () => {
    ...
    await http.put(
        `https://${SHOPIFY_AUTH.storeUrl}/admin/api/2021-04/themes/${liveTestingTheme.id}.json`,
...
afterEach(async () => {
    ...
    for (let i = 0; i < themeIdsToRemove.length; i++) {
        await http.delete(
            `https://${SHOPIFY_AUTH.storeUrl}/admin/api/2021-04/themes/${themeIdsToRemove[i]}.json`,
            {headers: {'X-Shopify-Access-Token': SHOPIFY_AUTH.password}}
        );
    }
    themeIdsToRemove = [];
...
afterAll(async () => {
    ...
    if (originallyLiveTheme) {
        await http.put(
            `https://${SHOPIFY_AUTH.storeUrl}/admin/api/2021-04/themes/${originallyLiveTheme.id}.json`,
    ...
    if (testingTheme) {
        await http.delete(
            `https://${SHOPIFY_AUTH.storeUrl}/admin/api/2021-04/themes/${testingTheme.id}.json`,
            {headers: {'X-Shopify-Access-Token': SHOPIFY_AUTH.password}}
        );
    }
    if (liveTestingTheme) {
